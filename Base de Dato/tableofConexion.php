...
$producto = 'CREATE TABLE IF NOT EXISTS productos (
    ...
    UNIQUE KEY unique_product (nombre, id_artesano)
)';

mysqli_query($conexion, $producto) or die("Error al crear la tabla productos: " . mysqli_error($conexion));

// Insertar datos en la tabla de productos
$insertarProductos = "INSERT INTO productos (nombre, nombre_artesano, id_artesano, subtitulo, precio, detalles, tallas, ruta, stock, created_at, updated_at, published_at, created_by_id, updated_by_id, tipo) VALUES
...
('Set de Regalo', NULL, NULL, 'Set de Regalo', 50.00, NULL, NULL, '52.png', 10, NULL,NULL, NULL, NULL, NULL, 2) ON DUPLICATE KEY UPDATE
  nombre = VALUES(nombre),
  nombre_artesano = VALUES(nombre_artesano),
  id_artesano = VALUES(id_artesano),
...
mysqli_query($conexion, $insertarProductos) or die("Error al insertar datos en la tabla productos: " . mysqli_error($conexion));
...
foreach ($datosPedidos as $pedido) {
    if (!pedido_existe($conexion, $pedido['fecha_pedidos'], $pedido['id_usuario'], $pedido['id_artesano'])) {
        $query = "INSERT INTO pedidos (fecha_pedidos, id_usuario, id_artesano, domicilio, gasto, productos_comprados, forma_pago, estado, created_at, updated_at, published_at, created_by_id, updated_by_id) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
        $stmt = mysqli_prepare($conexion, $query);
        mysqli_stmt_bind_param($stmt, 'siisississsii', 
