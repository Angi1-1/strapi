...
function CatalogoDeJoyeria() {
  ...
  const listProductosJoyeria = async () => {
    try {
      const response = await fetch(`http://localhost:1337/api/productos?filters[tipo]=3`, {
        method: 'GET'
      });
  ...
  const applyFiltersAndSort = (productos) => {
    let filteredProducts = productos.filter(producto => {
      const nombreMatches = filters.nombre === '' || producto.attributes.nombre.toLowerCase().includes(filters.nombre.toLowerCase());
      return nombreMatches;
    });

    if (filters.precioMayor10) {
      filteredProducts = filteredProducts.sort((a, b) => b.attributes.precio - a.attributes.precio);
    } else if (filters.precioMenor10) {
      filteredProducts = filteredProducts.sort((a, b) => a.attributes.precio - b.attributes.precio);
    }

    return filteredProducts;
  };

  const filteredAndSortedProducts = applyFiltersAndSort(listProduct);

  console.log("Productos filtrados y ordenados", filteredAndSortedProducts);
  return (
